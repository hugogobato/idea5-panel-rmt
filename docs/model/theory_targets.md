...
|---|---|---|---|---|---|---|
| T1 | DE risk formula for hard-threshold spectral SC + ridge-SC; boundary where risk stops improving (docs/model/frontier_ansatz.md made rigorous) | C1 upper side, C2 engine | BBP 2005 (T1); BGN 2011 outlier locations AND overlaps (T2); Dobriban-Wager 2018 ridge risk (T6); Hastie et al. 2022 ridgeless (T7) | Row-targeted single-unit risk instead of Frobenius; resolvent variant for treated-row projection; post-window transport term | WP-C1 overlay: F vs simulated RMSE to plotting accuracy across c-grid | ANSATZ STATED (Phase B, conjecture tag); proof dormant until G3 |
